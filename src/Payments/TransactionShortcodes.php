...
class TransactionShortcodes
{
    ...
    public function registerPaymentsShortcode($atts, $content = '')
    {
        ...
        if ($atts['type'] == 'all' || $atts['type'] == 'subscriptions') {
            ...
            if ($subscriptionsHtml) {
                wp_enqueue_script('fluentform_transactions', FLUENTFORMPRO_DIR_URL . 'public/js/fluentform_transactions_ui.js', ['jquery'], FLUENTFORM_VERSION, true);

                wp_enqueue_style(
                    'fluentform_transactions',
                    FLUENTFORMPRO_DIR_URL . 'public/css/fluentform_transactions.css',
                    [],
                    FLUENTFORM_VERSION
                );
    ...
    public function renderPaymentReceiptPage($data, $echo = true)
    {
        ...
        status_header(200);
        $file = FLUENTFORMPRO_DIR_PATH . 'src/views/frameless_view.php';
        extract($data);
        ob_start();
        include($file);
        echo  ob_get_clean();
